...
All required R packages are declared in the following script:  
`02_Scripts/01_Required_Packages_and_Setup.R`
This script defines the complete analytical environment for the project, ensuring consistency across all data processing and model execution steps.

During reproduction, **users do not need to run this setup script separately**,  as the same environment initialization has already been embedded within the main manuscript file:  
`04_Manuscript/final_manuscript.qmd`
...
These model scripts are located in:  
`02_Scripts/`
